...
if(errors.length===0){
 ...
 for(const token of["executive-analytics","install_report_defaults","organizations_install_report_defaults","report_metric_definitions","report_targets","csvExport","publicDashboards"])if(!moduleSeed.includes(token))errors.push(`Módulo incompleto: ${token}`);
...
console.log(`Etapa 16 validada: ${files.length} arquivos críticos, seis tabelas, duas views, RLS, integridade multi-tenant, metas, snapshots e exportação auditada.`);
