...
const migrations=[paths.schema,paths.balances,paths.movements,paths.reservations,...paths.assets,paths.security,paths.dashboard,paths.movementDetail,paths.entityDetail,paths.foundItems,paths.module,paths.creation,paths.hardening,paths.sensitiveColumns,paths.sensitiveWrite,paths.states];
const appFiles=[
...
for(const file of[...migrations,...appFiles])if(!fs.existsSync(file))errors.push(`Arquivo ausente: ${file}`);
...
if(!errors.length){
 const schema=read(paths.schema);const tables=[...schema.matchAll(/create table if not exists public\.(inventory_[a-z_]+)/gi)].map(match=>match[1]);
 ...
 const balances=read(paths.balances);const views=[...balances.matchAll(/create or replace view public\.(inventory_[a-z_]+)/gi)].map(match=>match[1]);
 for(const view of["inventory_stock_v","inventory_reserved_stock_v","inventory_available_stock_v","inventory_item_totals_v","inventory_asset_current_v","inventory_expiry_alerts_v"])if(!views.includes(view))errors.push(`View ausente: ${view}`);
 ...
 requireRegex(assets,[/select check_constraint\.constraint_name/i,/assign_inventory_asset/i,/return_inventory_asset/i,/start_inventory_stocktake/i,/submit_inventory_stocktake/i,/approve_inventory_stocktake/i,/post_inventory_stocktake_adjustment/i],"Ativos e inventários");
 const security=read(paths.security);if(new Set([...security.matchAll(/alter table public\.(inventory_[a-z_]+) enable row level security/gi)].map(match=>match[1])).size!==18)errors.push("RLS não cobre 18 tabelas.");
...
console.log(`Etapa 17 validada: ${migrations.length} migrations, 18 tabelas, 6 views, RLS, contratos seguros e ${appFiles.length} arquivos de aplicação/documentação.`);
