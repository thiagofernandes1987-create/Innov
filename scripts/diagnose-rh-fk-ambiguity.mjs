...
try{must(docker(["run","-d","--rm","--name",container,"-e","POSTGRES_PASSWORD=postgres","-e",`POSTGRES_DB=${db}`,"postgres:16-alpine"]),"postgres");let ready=false;for(let i=0;i<60;i++){const r=docker(["exec",container,"pg_isready","-U","postgres","-d",db]);if(r.status===0){ready=true;break;}Atomics.wait(new Int32Array(new SharedArrayBuffer(4)),0,0,500);}if(!ready)throw new Error("Postgres não ficou pronto");apply(container,db,"supabase/tests/rh/fixture.sql");for(const name of fs.readdirSync("supabase/migrations").filter(n=>/_rh_.*\.sql$/.test(n)).sort())apply(container,db,`supabase/migrations/${name}`);
const sql=`
with f as (
...
`;
const out=must(docker(["exec",container,"psql","-U","postgres","-d",db,"-Atc",sql]),"diagnóstico").stdout;fs.mkdirSync(".evidence/rh",{recursive:true});fs.writeFileSync(".evidence/rh/redundant-foreign-keys.txt",out);console.log(out||"Nenhuma FK simples redundante encontrada.");}finally{docker(["rm","-f",container],{timeout:20000});}
