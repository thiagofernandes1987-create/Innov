...
const files = [
  ...
  "supabase/migrations/20260804190000_stage22_communication_playbooks.sql",
  "supabase/tests/messaging-playbooks/playbooks.test.sql"
];
for (const file of files) if (!fs.existsSync(file)) throw new Error(`Arquivo ausente: ${file}`);
const database = process.env.MESSAGING_PLAYBOOKS_TEST_DB ?? "messaging_playbooks_test";
...
try {
  ...
  console.error("PostgreSQL indisponível: W-14 NÃO executada.");
...
try {
  for (const file of files) {
    ...
    for (const line of output.split("\n")) {
      ...
      if (!value) continue;
      console.log(value);
      if (/W-14 .*aprovad/i.test(value)) approvals += 1;
    }
  }
} catch (error) {
  console.error(details(error));
...
if (approvals !== 12) {
  console.error(`W-14 produziu ${approvals}/12 confirmações.`);
  process.exit(1);
}
console.log("Testes PostgreSQL W-14 aprovados: 12 controles.");
