...
for(const token of["finance_entry_balances_v","finance_cash_flow_monthly_v","finance-attachments","security_invoker=true","PROCUREMENT_ORDER","MEASUREMENT"])if(!schema.includes(token))errors.push(`Schema incompleto: ${token}`);
...
const engine=fs.readFileSync(files[4],"utf8");
...
console.log(`Etapa 15 validada: ${files.length} arquivos críticos, 11 tabelas, duas visões, RLS, aprovações, parcelas, liquidações, medições e fluxo de caixa.`);
