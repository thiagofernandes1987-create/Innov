...
export function validateIntegraContadorBase(value:string){const url=new URL(value);if(url.protocol!=="https:"||!ALLOWED_HOSTS.has(url.hostname)||url.username||url.password||url.search||url.hash)throw new Error("Host Integra Contador/SERPRO não permitido.");return url.origin;}
function safeBase(environment:IntegraContadorEnvironment){return validateIntegraContadorBase(envBase(environment));}
export function validateIntegraContadorServicePath(path:string){if(!path.startsWith("/")||path.startsWith("//")||path.includes("\\")||/[\r\n]/.test(path))throw new Error("Path de serviço Integra Contador inválido.");const u=new URL(path,"https://local.invalid");if(u.origin!=="https://local.invalid")throw new Error("Path externo não permitido.");return`${u.pathname}${u.search}`;}
...
export async function callIntegraContador<T>(input:{environment?:IntegraContadorEnvironment;path:string;method?:"GET"|"POST"|"PUT"|"DELETE";body?:unknown;headers?:Record<string,string>;correlationId?:string}):Promise<IntegraContadorResponse<T>>{
 const environment=input.environment??"VALIDATION";if(environment==="PRODUCTION"&&process.env.INTEGRA_CONTADOR_ENABLE_PRODUCTION!=="true")throw new Error("INTEGRA_CONTADOR_PRODUCTION_DISABLED");const base=safeBase(environment),path=validateIntegraContadorServicePath(input.path),token=await getIntegraContadorToken(environment),correlationId=input.correlationId?.trim()||randomUUID(),controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeoutMs());
...
export function getConfiguredIntegraContadorServices(){const events=process.env.INTEGRA_CONTADOR_DCTFWEB_EVENTS_PATH?.trim()||null,service=process.env.INTEGRA_CONTADOR_DCTFWEB_SERVICE_PATH?.trim()||null;return{dctfwebEventsPath:events?validateIntegraContadorServicePath(events):null,dctfwebServicePath:service?validateIntegraContadorServicePath(service):null};}
