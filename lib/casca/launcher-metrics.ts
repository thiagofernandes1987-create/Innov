...
function activeCount(collection: Row[], inactiveStatuses: readonly string[] = []): number {
  return collection.filter(item => !inactiveStatuses.includes(String(item.status ?? ""))).length;
}

function resultFailed(result: { error: unknown }): boolean {
...
export async function loadLauncherSummaries(context: SupabaseClient): Promise<LauncherSummaryMap> {
  ...
  if (!resultFailed(leadsResult) && !resultFailed(opportunitiesResult)) {
    ...
    summaries.crm = {
      ...
      support: `${formatCompactCurrency(pipelineValue)} no pipeline`,
      ...
      recent: recentOpportunities(opportunities),
      available: true
    };
  }

  if (!resultFailed(clientsResult)) {
    ...
    summaries.clientes = {
      ...
      support: `${formatCompactNumber(clients.length)} cadastrados`,
      progress: clampProgress(clients.length ? (active / clients.length) * 100 : 0),
      available: true
    };
  }

  if (!resultFailed(projectsResult)) {
    const projects = rows(projectsResult.data);
    const active = activeCount(projects, ["COMPLETED", "CANCELED", "ARCHIVED"]);
    const averageProgress = projects.length
      ? projects.reduce((sum, item) => sum + numeric(item.progress), 0) / projects.length
      : 0;
    summaries.obras = {
      label: "Obras em andamento",
      value: formatCompactNumber(active),
      support: `${formatCompactNumber(projects.length)} na carteira`,
      progress: clampProgress(averageProgress * 100),
      available: true
    };
  }

  if (!resultFailed(baselinesResult) && !resultFailed(milestonesResult)) {
    ...
    summaries.planejamento = {
      label: "Cronogramas controlados",
      value: formatCompactNumber(baselines.length),
      support: `${upcoming} marco(s) futuro(s)`,
      secondaryLabel: "Baselines congeladas",
      secondaryValue: formatCompactNumber(frozen),
      progress: clampProgress(baselines.length ? (frozen / baselines.length) * 100 : 0),
      available: true
    };
  }

  if (!resultFailed(tasksResult)) {
    ...
    summaries.tarefas = {
      ...
      value: formatCompactNumber(open),
      support: `${overdue} atrasada(s)`,
      progress: clampProgress(open ? ((open - overdue) / open) * 100 : 100),
      available: true
    };
  }

  if (!resultFailed(dailyLogsResult)) {
    const latest = rows(dailyLogsResult.data)[0];
    const project = first(latest?.projects);
    summaries.diario = {
      label: "Registros de obra",
      value: formatCompactNumber(Number(dailyLogsResult.count ?? 0)),
      support: latest ? `Último em ${new Intl.DateTimeFormat("pt-BR").format(new Date(`${String(latest.log_date)}T12:00:00`))}` : "Nenhum registro",
      secondaryLabel: project ? String(project.code ?? "Obra") : null,
      secondaryValue: project ? String(project.name ?? "") : null,
      progress: latest ? 100 : 0,
      available: true
    };
  }

  if (!resultFailed(teamsResult) && !resultFailed(membersResult) && !resultFailed(resourcesResult)) {
    ...
    summaries.equipes = {
      ...
      support: `${activeTeams} equipe(s) ativa(s)`,
      secondaryLabel: "Recursos ativos",
      secondaryValue: formatCompactNumber(activeResources),
      progress: clampProgress(teams.length ? (activeTeams / teams.length) * 100 : 0),
      available: true
    };
  }

  if (!resultFailed(budgetsResult)) {
    ...
    summaries.orcamentos = {
      label: "Orçamentos na carteira",
      value: formatCompactNumber(budgets.length),
      support: `${formatCompactCurrency(total)} orçados`,
      secondaryLabel: "Aprovação pendente",
      secondaryValue: formatCompactNumber(pending),
      progress: clampProgress(budgets.length ? ((budgets.length - pending) / budgets.length) * 100 : 0),
      available: true
    };
  }

  if (!resultFailed(proposalsResult)) {
    ...
    summaries.propostas = {
      label: "Propostas",
      value: formatCompactNumber(proposals.length),
      support: `${sent} enviada(s) ou concluída(s)`,
      secondaryLabel: "Descontos pendentes",
      secondaryValue: formatCompactNumber(pendingDiscount),
      progress: clampProgress(proposals.length ? (sent / proposals.length) * 100 : 0),
      available: true
    };
  }

  if (!resultFailed(contractsResult)) {
    ...
    summaries.contratos = {
      ...
      value: formatCompactNumber(active),
      support: `${formatCompactNumber(contracts.length)} no histórico`,
      progress: clampProgress(contracts.length ? (active / contracts.length) * 100 : 0),
      available: true
    };
  }

  if (!resultFailed(documentsResult)) {
    const documents = rows(documentsResult.data);
    const released = documents.filter(item => String(item.status) === "RELEASED").length;
    summaries.documentos = {
      label: "Documentos",
      value: formatCompactNumber(documents.length),
      support: `${released} liberado(s)`,
      progress: clampProgress(documents.length ? (released / documents.length) * 100 : 0),
      available: true
    };
  }
