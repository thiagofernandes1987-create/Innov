...
async function inspect(target: string, cookie: string | null) {
  ...
  try {
    ...
    const selectedHeaders = Object.fromEntries(
      [...response.headers.entries()].filter(([name]) => [
        "location", "set-cookie", "content-type", "content-length", "content-range", "content-disposition", "server", "via", "x-cache"
      ].includes(name.toLowerCase()))
    );
    return {
      target,
      cookie: cookie ?? "sem cookie",
    ...
    return { target, cookie: cookie ?? "sem cookie", error: error instanceof Error ? `${error.name}: ${error.message}` : "erro" };
  } finally {
    clearTimeout(timer);
  }
}
...
export async function GET(request: Request) {
  ...
  for (const target of targets) {
    results.push(await inspect(target, null));
    results.push(await inspect(target, "security=true"));
  }
