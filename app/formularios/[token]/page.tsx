...
export default async function PublicQualityForm({params,searchParams}:{params:Promise<{token:string}>;searchParams:Promise<{error?:string;submitted?:string}>}){
  ...
  if(error||!link||link.revoked_at||(link.expires_at&&new Date(link.expires_at).getTime()<Date.now()))notFound();
  ...
  const template=Array.isArray((version as any).quality_form_templates)?(version as any).quality_form_templates[0]:(version as any).quality_form_templates;const schema=parseQualityFormSchema(version.schema_json);
