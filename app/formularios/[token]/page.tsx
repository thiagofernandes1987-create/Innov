...
export default async function PublicQualityForm({params,searchParams}:{params:Promise<{token:string}>;searchParams:Promise<{error?:string;submitted?:string}>}){
  ...
  const{data:version}=await admin.from("quality_form_versions").select("schema_json,quality_form_templates!quality_form_versions_template_id_fkey(title,description,kind)").eq("id",assignment.template_version_id).single();if(!version)notFound();
