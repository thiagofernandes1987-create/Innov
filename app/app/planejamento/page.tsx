...
export default async function PlanningPage() {
  ...
  return (
    <main className="content">
      <div className="page-head"><div><h1>Planejamento</h1><p className="muted">Prazo, progresso e impedimentos por obra. O código abre o cronograma.</p></div></div>
      {error ? <div className="validation blocking">{error.message}</div> : null}
      <section className="card table-wrap">
        <table><thead><tr><th>Obra</th><th>Cliente</th><th>Status</th><th>Progresso</th><th>Período</th><th>Situação do prazo</th><th>Atrasadas</th><th>Bloqueadas</th></tr></thead><tbody>
          {rows.map((project) => <tr key={project.id}><td><Link href={`/app/obras/${project.id}/cronograma`}><strong>{project.code}</strong><br /><span className="muted">{project.name}</span></Link></td><td>{project.client?.trade_name || project.client?.legal_name || "—"}</td><td><span className={statusBadge(project.status)}>{project.status}</span></td><td style={{ minWidth: 160 }}><div className="progress-row"><div><span>{formatPercent(project.progress)}</span></div><div className="progress-track"><div className="progress-fill" style={{ width: formatPercent(project.progress) }} /></div></div></td><td>{formatDate(project.planned_start)} → {formatDate(project.planned_end)}</td><td>{prazoDaObra(project.planned_end, project.overdue)}</td><td>{project.overdue ? <span className="badge badge-danger">{project.overdue}</span> : "0"}</td><td>{project.blocked ? <span className="badge badge-warning">{project.blocked}</span> : "0"}</td></tr>)}
          {!rows.length ? <tr><td colSpan={8}>Nenhuma obra no portfólio.</td></tr> : null}
        </tbody></table>
      </section>
