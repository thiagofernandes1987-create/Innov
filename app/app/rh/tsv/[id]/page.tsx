...
export default async function TsvDetailPage({
  params,
  searchParams
}: {
  ...
  const [{ data: employment, error }, { data: profile }, { data: event }] = await Promise.all([
    ...
    context.supabase
      ...
      .select("id,status,receipt_number,created_at")
      .eq("organization_id", context.organizationId)
      .eq("event_type", "S-2300")
      .eq("source_type", "TSV_EMPLOYMENT")
      .eq("source_id", id)
  ...
  const conditions = Array.isArray(employment.rh_employment_conditions)
    ? [...employment.rh_employment_conditions]
    : [];
  const condition =
    conditions.sort((a, b) => String(b.valid_from ?? "").localeCompare(String(a.valid_from ?? "")))[0] ?? null;
  ...
  return (
    <main className="content">
      <section className="page-heading">
        <div>
          <span className="badge">TSVE · {employment.registration_number}</span>
          <h1>{person?.preferred_name || person?.full_name || worker?.worker_code}</h1>
          <p>
            Início {employment.admission_date} · {employment.employment_type}. O S-2300 categoria 721 exige
            remuneração, FGTS, cargo/CBO e local de trabalho.
          </p>
        </div>
        <Link className="button button-secondary" href="/app/rh/tsv">
          Voltar
        </Link>
      </section>
      ...
      <section className="stats-grid">
        ...
        <article className="card"><small>CATEGORIA</small><strong>{profile?.category_code ?? "721"}</strong></article>
        <article className="card"><small>LOCAL</small><strong>{establishment?.name ?? "ausente"}</strong></article>
        <article className="card"><small>S-2300</small><strong>{event?.status ?? "NÃO GERADO"}</strong></article>
      </section>
      ...
      <form action={saveTsvEsocialProfile} className="relationship-form">
        ...
        <section className="card card-pad">
          ...
          <div className="form-grid">
            <label><span>Tipo logradouro</span><input name="streetType" defaultValue={profile?.street_type ?? "R"} /></label>
            <label className="span-2"><span>Logradouro *</span><input name="street" required defaultValue={profile?.street ?? ""} /></label>
            <label><span>Número *</span><input name="streetNumber" required defaultValue={profile?.street_number ?? ""} /></label>
            <label><span>Complemento</span><input name="complement" defaultValue={profile?.complement ?? ""} /></label>
            <label><span>Bairro</span><input name="neighborhood" defaultValue={profile?.neighborhood ?? ""} /></label>
            <label><span>CEP *</span><input name="postalCode" required defaultValue={profile?.postal_code ?? ""} /></label>
            <label><span>Município IBGE *</span><input name="cityIbgeCode" required defaultValue={profile?.city_ibge_code ?? ""} /></label>
            <label><span>UF *</span><input name="stateCode" required maxLength={2} defaultValue={profile?.state_code ?? "SP"} /></label>
          </div>
        </section>

        <section className="card card-pad">
          <span className="eyebrow">IMIGRANTE</span>
          <div className="form-grid">
            <label><span>Tempo residência</span><select name="immigrantResidenceTerm" defaultValue={profile?.immigrant_residence_term ?? ""}><option value="">Não informar</option><option value="1">Indeterminado</option><option value="2">Determinado</option></select></label>
            <label><span>Condição ingresso</span><select name="immigrantEntryCondition" defaultValue={profile?.immigrant_entry_condition ?? ""}><option value="">Não informar</option>{[1, 2, 3, 4, 5, 6, 7].map((value) => <option value={value} key={value}>{value}</option>)}</select></label>
          </div>
        </section>

        <section className="card card-pad">
          <span className="eyebrow">REMUNERAÇÃO / FGTS</span>
          <div className="form-grid">
            ...
            <label><span>Data opção FGTS *</span><input name="fgtsOptionDate" type="date" required defaultValue={profile?.fgts_option_date ?? employment.admission_date} /></label>
          ...
          <p className="validation">
            Cargo/CBO, empregador, estabelecimento e remuneração base são lidos da condição vigente do vínculo:
            {condition ? ` ${position?.name ?? "cargo ausente"} · ${employer?.legal_name ?? "empresa"}` : " condição ainda não cadastrada"}.
          </p>
        </section>
      ...
      <section className="card card-pad">
        <span className="eyebrow">PRODUÇÃO RESTRITA</span>
        <h2>Gerar e assinar S-2300</h2>
        <form action={generateS2300}>
          <input type="hidden" name="employmentId" value={employment.id} />
          <input type="hidden" name="environment" value="RESTRICTED" />
          <button className="button button-primary" type="submit" disabled={!profile || !condition}>Gerar + assinar S-2300</button>
        </form>
