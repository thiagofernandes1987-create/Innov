...
export default async function ClientFormDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string;submitted?:string}>}){
  ...
  const{data:version}=await admin.from("quality_form_versions").select("schema_json,version_number,quality_form_templates(title,description,kind)").eq("id",assignment.template_version_id).single();if(!version)notFound();
  const schema=parseQualityFormSchema(version.schema_json);const template=Array.isArray((version as any).quality_form_templates)?(version as any).quality_form_templates[0]:(version as any).quality_form_templates;
