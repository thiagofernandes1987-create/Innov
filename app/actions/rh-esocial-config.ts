...
export async function createEsocialEmployerProfile(data:FormData){
 ...
 const{error}=await context.supabase.from("rh_esocial_employer_profiles").insert(payload);if(error)fail(mensagemDeFalha("rh-esocial-config.createEsocialEmployerProfile", error));revalidatePath(PATH);redirect(PATH);
}
...
export async function createEsocialEstablishmentProfile(data:FormData){
 ...
 const{error}=await context.supabase.from("rh_esocial_establishment_profiles").insert(payload);if(error)fail(mensagemDeFalha("rh-esocial-config.createEsocialEstablishmentProfile", error));revalidatePath(PATH);redirect(PATH);
}
...
export async function createEsocialTaxAllocationProfile(data:FormData){
 ...
 const{error}=await context.supabase.from("rh_esocial_tax_allocation_profiles").insert(payload);if(error)fail(mensagemDeFalha("rh-esocial-config.createEsocialTaxAllocationProfile", error));revalidatePath(PATH);redirect(PATH);
}
...
export async function configureEsocialRubricVersion(data:FormData){
 ...
 if(error)fail(mensagemDeFalha("rh-esocial-config.configureEsocialRubricVersion", error));revalidatePath(PATH);redirect(PATH);
}
