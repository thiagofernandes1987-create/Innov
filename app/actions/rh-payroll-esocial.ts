...
async function persist(context:RhContext,input:{eventType:string;sourceType:string;sourceId:string;eventGroup:number;environment:EsocialEnvironment;employerType:1|2;employerNumber:string;eventKey:string;unsignedXml:string;signedXml:string;signedSha256:string}){
 ...
 if(error)throw new Error(mensagemDeFalha("rh-payroll-esocial.persist", error));return String(data);
}
...
async function periodContext(context:RhContext,periodId:string){
 ...
 if(error||!period)throw new Error(mensagemDeFalha("rh-payroll-esocial.periodContext", error, "Competência não encontrada."));
 ...
 if(rError||!run)throw new Error(mensagemDeFalha("rh-payroll-esocial.periodContext", rError, "Execução concluída não encontrada."));
...
async function employerForEmployment(context:RhContext,employmentId:string,asOf:string){
 ...
 if(cError||!condition?.employer_id)throw new Error(mensagemDeFalha("rh-payroll-esocial.employerForEmployment", cError, "Condição vigente/empregador não encontrado."));
 ...
 if(eError||!employer?.tax_id)throw new Error(mensagemDeFalha("rh-payroll-esocial.employerForEmployment", eError, "Empregador sem inscrição fiscal."));
 return employer;
}

export async function createPayrollPaymentOrders(data:FormData){const context=await requireCapability("rh","update"),periodId=text(data,"periodId"),path=`${BASE}/competencias/${periodId}`;const{data:count,error}=await context.supabase.rpc("create_rh_payroll_payments",{p_period_id:periodId});if(error)fail(path,mensagemDeFalha("rh-payroll-esocial.createPayrollPaymentOrders", error));redirect(`${path}?success=${encodeURIComponent(`${count??0} ordem(ns) de pagamento criada(s).`)}`);}

export async function settlePayrollPayment(data:FormData){const context=await requireCapability("rh","approve"),paymentId=text(data,"paymentId"),periodId=text(data,"periodId"),path=`${BASE}/competencias/${periodId}`;const{error}=await context.supabase.rpc("settle_rh_payroll_payment",{p_payment_id:paymentId,p_paid_at:text(data,"paidAt"),p_bank_reference:text(data,"bankReference")||null,p_external_payment_id:text(data,"externalPaymentId")||null,p_evidence_reference:text(data,"evidenceReference")||null});if(error)fail(path,mensagemDeFalha("rh-payroll-esocial.settlePayrollPayment", error));redirect(path);}
...
export async function generateS1200(data:FormData){
 ...
 try{
  ...
  const{data:wr,error}=await context.supabase.from("rh_payroll_worker_results").select("id,employment_id").eq("organization_id",context.organizationId).eq("id",workerResultId).eq("run_id",run.id).maybeSingle();if(error||!wr)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1200", error, "Resultado do trabalhador não encontrado."));
  const{data:employment,error:employmentError}=await context.supabase.from("rh_employments").select("worker_id,registration_number").eq("organization_id",context.organizationId).eq("id",wr.employment_id).maybeSingle();if(employmentError||!employment)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1200", employmentError, "Vínculo não encontrado."));
  const{data:worker,error:workerError}=await context.supabase.from("rh_workers").select("person_id").eq("organization_id",context.organizationId).eq("id",employment.worker_id).maybeSingle();if(workerError||!worker)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1200", workerError, "Trabalhador não encontrado."));
  const{data:person,error:personError}=await context.supabase.from("rh_people").select("cpf").eq("organization_id",context.organizationId).eq("id",worker.person_id).maybeSingle();if(personError||!person?.cpf)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1200", personError, "CPF ausente."));
  const{data:condition,error:conditionError}=await context.supabase.from("rh_employment_conditions").select("employer_id,establishment_id,tax_allocation_id,esocial_category_code").eq("organization_id",context.organizationId).eq("employment_id",wr.employment_id).lte("valid_from",period.reference_month).order("valid_from",{ascending:false}).limit(1).maybeSingle();if(conditionError||!condition?.esocial_category_code||!condition.tax_allocation_id)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1200", conditionError, "Condição vigente sem categoria/lotação eSocial."));
  ...
  if(empError||estError||lotError||lError)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1200", empError)??mensagemDeFalha("rh-payroll-esocial.generateS1200", estError)??mensagemDeFalha("rh-payroll-esocial.generateS1200", lotError)??mensagemDeFalha("rh-payroll-esocial.generateS1200", lError, "Falha ao carregar estrutura eSocial."));if(!employer||!est||!lot?.esocial_lotacao_code)throw new Error("Estrutura eSocial incompleta.");
 ...
 }catch(error){fail(path,error instanceof Error?mensagemDeFalha("rh-payroll-esocial.generateS1200", error):"Falha ao gerar S-1200.");}
...
export async function generateS1210(data:FormData){
 ...
 try{
  const{period,run,month}=await periodContext(context,periodId);const{data:p,error}=await context.supabase.from("rh_payroll_payments").select("id,worker_result_id,employment_id,amount,paid_at,status").eq("organization_id",context.organizationId).eq("id",paymentId).eq("run_id",run.id).maybeSingle();if(error||!p)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1210", error, "Pagamento não encontrado."));if(p.status!=="SETTLED"||!p.paid_at)throw new Error("S-1210 só pode ser gerado de pagamento liquidado.");
  ...
  const{data:employment,error:employmentError}=await context.supabase.from("rh_employments").select("worker_id").eq("organization_id",context.organizationId).eq("id",p.employment_id).maybeSingle();if(employmentError||!employment)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1210", employmentError, "Vínculo não encontrado."));
  const{data:worker,error:workerError}=await context.supabase.from("rh_workers").select("person_id").eq("organization_id",context.organizationId).eq("id",employment.worker_id).maybeSingle();if(workerError||!worker)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1210", workerError, "Trabalhador não encontrado."));
  const{data:person,error:personError}=await context.supabase.from("rh_people").select("cpf").eq("organization_id",context.organizationId).eq("id",worker.person_id).maybeSingle();if(personError||!person?.cpf)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1210", personError, "CPF ausente."));
 ...
 }catch(error){fail(path,error instanceof Error?mensagemDeFalha("rh-payroll-esocial.generateS1210", error):"Falha ao gerar S-1210.");}
 redirect(`/app/rh/obrigacoes/esocial/eventos/${eventId}`);
}
...
export async function generateS1299(data:FormData){
 ...
 try{
  const{period,run,month}=await periodContext(context,periodId);const{data:workerRows,count:workers,error:workersError}=await context.supabase.from("rh_payroll_worker_results").select("id,employment_id",{count:"exact"}).eq("organization_id",context.organizationId).eq("run_id",run.id);if(workersError)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1299", workersError));if(!workerRows?.length)throw new Error("S-1299 exige ao menos um resultado de trabalhador para identificar o empregador desta competência.");
  ...
  const{data:payments,error:paymentsError}=await context.supabase.from("rh_payroll_payments").select("id,status").eq("organization_id",context.organizationId).eq("run_id",run.id);if(paymentsError)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1299", paymentsError));if((payments??[]).some(p=>p.status!=="SETTLED"))throw new Error("Fechamento exige todos pagamentos liquidados.");
 ...
 }catch(error){fail(path,error instanceof Error?mensagemDeFalha("rh-payroll-esocial.generateS1299", error):"Falha ao gerar S-1299.");}
 redirect(`/app/rh/obrigacoes/esocial/eventos/${eventId}`);
}
...
export async function generateS1298(data:FormData){
 ...
 try{
  ...
  const{data:firstWr,error:firstError}=await context.supabase.from("rh_payroll_worker_results").select("employment_id").eq("organization_id",context.organizationId).eq("run_id",run.id).limit(1).maybeSingle();if(firstError||!firstWr)throw new Error(mensagemDeFalha("rh-payroll-esocial.generateS1298", firstError, "Não foi possível identificar o empregador da competência."));
 ...
 }catch(error){fail(path,error instanceof Error?mensagemDeFalha("rh-payroll-esocial.generateS1298", error):"Falha ao gerar S-1298.");}
 redirect(`/app/rh/obrigacoes/esocial/eventos/${eventId}`);
}
