...
function failData(path: string, operation: string, error: ProviderLikeError | null | undefined, message: string): never { reportDataAccessError(`operational-finance.${operation}`, error); fail(path, message); }
...
export async function createFinanceEntry(data: FormData) {
  ...
  try { installments = parseInstallments(text(data, "installmentsJson")); } catch (validationError) { fail("/app/financeiro/lancamentos/novo", validationError instanceof Error ? validationError.message : "Parcelas inválidas."); }
  ...
  if (error || !entry) failData("/app/financeiro/lancamentos/novo", "create-entry", error, "Não foi possível criar o lançamento.");
  ...
  if (installmentError) {
    reportDataAccessError("operational-finance.create-installments", installmentError);
    const { error: cleanupError } = await context.supabase.from("finance_entries").delete().eq("id", entry.id);
    reportDataAccessError("operational-finance.cleanup-entry", cleanupError);
    fail("/app/financeiro/lancamentos/novo", "Não foi possível criar as parcelas do lançamento.");
  }
  redirect(`/app/financeiro/lancamentos/${entry.id}`);
}

export async function submitFinanceEntry(data: FormData) { const id = text(data, "entryId"); const context = await requireCapability("financeiro", "update"); const { error } = await context.supabase.rpc("submit_finance_entry", { p_entry_id: id }); if (error) failData(`/app/financeiro/lancamentos/${id}`, "submit-entry", error, "Não foi possível enviar o lançamento para aprovação."); revalidatePath(`/app/financeiro/lancamentos/${id}`); revalidatePath("/app/financeiro"); }
export async function decideFinanceApproval(data: FormData) { const id = text(data, "entryId"); const context = await requireCapability("financeiro", "approve"); const { error } = await context.supabase.rpc("decide_finance_approval", { p_approval_id: text(data, "approvalId"), p_decision: text(data, "decision"), p_comment: text(data, "comment") }); if (error) failData(`/app/financeiro/lancamentos/${id}`, "decide-approval", error, "Não foi possível registrar a decisão de aprovação."); revalidatePath(`/app/financeiro/lancamentos/${id}`); revalidatePath("/app/financeiro"); }
...
export async function registerFinanceSettlement(data: FormData) {
  ...
  if (file instanceof File && file.size > 0) {
    ...
    try {
      ...
      fail(`/app/financeiro/lancamentos/${entryId}`, fileSecurityMessage(uploadError, { allowedLabel: "PDF, XLSX, JPG, PNG ou WebP" }));
    }
  }
  ...
  if (error) {
    reportDataAccessError("operational-finance.register-settlement", error);
    if (uploadedPath) {
      const { error: cleanupError } = await admin.storage.from("finance-attachments").remove([uploadedPath]);
      reportDataAccessError("operational-finance.cleanup-settlement-artifact", cleanupError);
    }
    fail(`/app/financeiro/lancamentos/${entryId}`, "Não foi possível registrar a liquidação.");
  }
...
export async function createFinanceMeasurement(data: FormData) {
  ...
  try { items = parseMeasurementItems(text(data, "itemsJson")); } catch (validationError) { fail("/app/financeiro/medicoes/nova", validationError instanceof Error ? validationError.message : "Itens inválidos."); }
  ...
  if (error || !measurement) failData("/app/financeiro/medicoes/nova", "create-measurement", error, "Não foi possível criar a medição.");
  ...
  if (itemsError) {
    reportDataAccessError("operational-finance.create-measurement-items", itemsError);
    const { error: cleanupError } = await context.supabase.from("finance_measurements").delete().eq("id", measurement.id);
    reportDataAccessError("operational-finance.cleanup-measurement", cleanupError);
    fail("/app/financeiro/medicoes/nova", "Não foi possível criar os itens da medição.");
  }
  redirect(`/app/financeiro/medicoes/${measurement.id}`);
}
export async function submitFinanceMeasurement(data: FormData) { const id = text(data, "measurementId"); const context = await requireCapability("financeiro", "update"); const { error } = await context.supabase.rpc("submit_finance_measurement", { p_measurement_id: id }); if (error) failData(`/app/financeiro/medicoes/${id}`, "submit-measurement", error, "Não foi possível enviar a medição para aprovação."); revalidatePath(`/app/financeiro/medicoes/${id}`); }
export async function decideFinanceMeasurement(data: FormData) { const id = text(data, "measurementId"); const context = await requireCapability("financeiro", "approve"); const { error } = await context.supabase.rpc("decide_finance_measurement", { p_measurement_id: id, p_decision: text(data, "decision"), p_comment: text(data, "comment") }); if (error) failData(`/app/financeiro/medicoes/${id}`, "decide-measurement", error, "Não foi possível registrar a decisão da medição."); revalidatePath(`/app/financeiro/medicoes/${id}`); revalidatePath("/app/financeiro/lancamentos"); }
