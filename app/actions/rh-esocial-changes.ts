...
async function persist(context:Awaited<ReturnType<typeof requireCapability>>,input:{eventType:string,eventKey:string;sourceType:string;sourceId:string;environment:EsocialEnvironment;employerType:1|2;employerNumber:string;unsignedXml:string;signedXml:string;signedSha256:string}){
 ...
 if(error)throw new Error(mensagemDeFalha("rh-esocial-changes.persist", error));return String(data);
}
...
export async function createRhPersonalEsocialChange(data:FormData){
 ...
 if(error)fail(workerId,mensagemDeFalha("rh-esocial-changes.createRhPersonalEsocialChange", error));redirect(`${page(workerId)}?createdProfile=${encodeURIComponent(String(profileId))}`);
}
...
export async function createRhContractEsocialChange(data:FormData){
 ...
 if(error)fail(workerId,mensagemDeFalha("rh-esocial-changes.createRhContractEsocialChange", error));redirect(`${page(workerId)}?createdCondition=${encodeURIComponent(String(conditionId))}`);
}
...
export async function generateRhEsocialChangeEvent(data:FormData){
 ...
 try{
  ...
  const{data:worker,error:wError}=await context.supabase.from("rh_workers").select("id,person_id").eq("organization_id",context.organizationId).eq("id",workerId).maybeSingle();if(wError||!worker)throw new Error(mensagemDeFalha("rh-esocial-changes.generateRhEsocialChangeEvent", wError, "Trabalhador não encontrado."));
  const{data:person,error:pError}=await context.supabase.from("rh_people").select("cpf").eq("organization_id",context.organizationId).eq("id",worker.person_id).maybeSingle();if(pError||!person?.cpf)throw new Error(mensagemDeFalha("rh-esocial-changes.generateRhEsocialChangeEvent", pError, "CPF do trabalhador ausente."));
  if(eventType==="S-2205"){
   ...
   ]);if(prError||!profile)throw new Error(mensagemDeFalha("rh-esocial-changes.generateRhEsocialChangeEvent", prError, "Versão cadastral não encontrada."));if(cError||!condition)throw new Error(mensagemDeFalha("rh-esocial-changes.generateRhEsocialChangeEvent", cError, "Condição contratual vigente não encontrada."));
   const{data:employer,error:eError}=await context.supabase.from("rh_employers").select("tax_id").eq("organization_id",context.organizationId).eq("id",condition.employer_id).maybeSingle();if(eError||!employer)throw new Error(mensagemDeFalha("rh-esocial-changes.generateRhEsocialChangeEvent", eError, "Empregador não encontrado."));const eType=employerType(employer.tax_id);
   ...
   const{data:condition,error:cError}=await context.supabase.from("rh_employment_conditions").select("id,employment_id,valid_from,employer_id,establishment_id,position_id,function_id,union_id,work_schedule_id,base_salary,esocial_category_code,change_reason").eq("organization_id",context.organizationId).eq("id",sourceId).maybeSingle();if(cError||!condition)throw new Error(mensagemDeFalha("rh-esocial-changes.generateRhEsocialChangeEvent", cError, "Condição contratual não encontrada."));
 ...
 }catch(error){fail(workerId,error instanceof Error?mensagemDeFalha("rh-esocial-changes.generateRhEsocialChangeEvent", error):"Falha ao gerar alteração eSocial.");}
