...
export async function setProfileAccessLevel(formData:FormData){
 ...
 const{error}=await context.supabase.rpc("set_profile_module_permission",{
  p_profile_id:profileId,p_module_key:moduleKey,p_access_level:toDatabaseAccessLevel(level),
  p_can_approve:full,p_can_release:full,p_can_sign:full,p_can_export:editable,
  p_can_administer:full,p_can_view_sensitive:full,p_reason:reason
 });
 if(error)fail("/app/administracao/perfis",error.message);
