...
export async function importRhRegulatoryTemplate(data:FormData){
 ...
 if(error)fail(error.message);done(`Parâmetro ${code} criado a partir do template oficial.`);
}
...
export async function createRhPayrollBase(data:FormData){
 ...
 if(error)fail(error.message);done(`Base ${code} criada.`);
}
...
export async function addRhPayrollBaseMember(data:FormData){
 ...
 if(error)fail(error.message);revalidatePath(PATH);done("Rubrica adicionada à composição da base.");
}
...
export async function createRhDerivedRubric(data:FormData){
 ...
 if(error)fail(error.message);done(`Rubrica automática ${code} criada.`);
}
