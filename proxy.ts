...
const legacyModules:Record<string,readonly string[]>={
  COMERCIAL:["crm","clientes","propostas","contratos","assinaturas"],
  FINANCEIRO:["financeiro","orcamentos","clientes","contratos","aditivos","assinaturas","documentos"],
  ORCAMENTISTA:["orcamentos","clientes","documentos"],
  GESTOR_OBRAS:["clientes","obras","planejamento","tarefas","diario","equipes","documentos"],
  ENGENHEIRO:["obras","planejamento","tarefas","diario","documentos"],
  QUALIDADE:["obras","diario","documentos","qualidade"],
  SAC:["clientes","obras","sac"]
};
...
export async function proxy(request:NextRequest){
  ...
  if(!supabaseUrl||!supabaseKey){
    ...
    const url=request.nextUrl.clone();url.pathname="/login";url.searchParams.set("error","Autenticação ainda não configurada neste ambiente.");return NextResponse.redirect(url);
  }
