...
- [ ] T-43.3 — Fase 2 da §36 — construir a camada de execução em Go, padronizar queue/retry, instrumentar OpenTelemetry. Plano em `docs/superpowers/plans/2026-08-10-camada-de-execucao-go.md`
  ...
  - [ ] **Tarefa 5 — AUTORIZADA, bloqueada por credencial.** O proprietário autorizou aplicar as `stage22_*` em 10/08/2026. **O que falta é `SUPABASE_DB_URL` no ambiente da sessão.** São 25 migrations, 262 KB de SQL; aplicá-las pelo MCP exigiria que todo esse texto passasse pelo contexto do modelo duas vezes — ler e reescrever no parâmetro —, e uma cadeia interrompida no meio deixa policy derrubada sem recriação em tabela de produção. Com a variável, o caminho é o que o próprio `migrations-aplicadas.json` documenta: `psql -f` na ordem dos carimbos e depois `pnpm ledger:atualizar`. **Levantamento de risco já feito:** as tabelas `whatsapp_*` têm **0 linhas**, então os backfills são no-op e nenhuma constraint `not null` tem dado que a viole; os dois `delete from` estão dentro de corpo de função (rotação de segredo e liberação de lease); os `drop` são de trigger/policy que as próprias migrations recriam na mesma transação, e cada arquivo abre com `begin;`
  - [ ] Implementação PostgREST, depois das migrations. Medido em 10/08: as filas **não existem no banco** — 0 de 5 tabelas, 0 de 8 RPCs, contra 256 tabelas no `public`. `stage22_multiprovider_storage` e `stage22_outbox_delivery` estão no débito congelado, que condiciona a saída a *"deploy de código compatível, aplicação controlada e testes DB"*. **Nenhum consumidor pode ser ligado antes disso, e a aplicação é decisão do responsável nomeado**
