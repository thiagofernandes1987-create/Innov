...
- [ ] T-37.13 — **Mais de uma UF continua em aberto, e não por esquema.** `cost_reference_snapshots` já tem `region` na chave natural; o que falta é fonte: o SindusCon-SP publica São Paulo, e cada estado tem o seu sindicato com página, formato e data-base próprios. Não é semear — é uma integração por entidade. Decidir se vale por UF sob demanda (a construtora informa onde atua) ou se o CUB fora de SP entra por importação manual com evidência anexada
- [ ] T-37.14 — O `cron` do CUB ainda lê só a notícia, e agora o banco tem dezenove tipologias que ele não sabe atualizar: em agosto, dezoito ficam paradas em 07/2026 e uma avança. Ligar `lerSerieHistoricaDoCub` ao caminho agendado, com a mesma guarda de data-base que já existe
...
- [ ] T-37.12 — **O catálogo SINAPI é gravável por qualquer membro interno.** Descoberto ao conferir a leitura da T-37.10: as quatro tabelas (`cost_compositions`, `cost_composition_versions`, `cost_composition_items`, `cost_catalog_items`) têm política única `for all using (is_internal_member(organization_id))`, e `authenticated` tem `INSERT, UPDATE, DELETE` no `grant`. `is_internal_member` só exclui `CLIENTE` — então montador, assistente e qualquer outro papel interno podem reescrever preço publicado direto pelo PostgREST. A versão nasce `FROZEN` e nada impede o `update`. A escrita deveria ser exclusiva de `service_role`, como já é nas RPCs de importação: leitura para membro interno, gravação só pelo lote. Medido em 4 de agosto; `anon` está barrado pela política, não pelo privilégio
- [ ] T-37.11 — Remover `stage19_can_read_global_diagnostics()`, pendência da FND-0010 da auditoria APEX. A migration da auditoria a dropava supondo o hardening R3B aplicado; medido em 4 de agosto, a policy `observability_diagnostics_select` ainda a chama, porque `r3b_observability_security_hardening` está no débito congelado da S-22. A função não é órfã: é load-bearing. Depende de a S-22 aplicar o R3B primeiro
