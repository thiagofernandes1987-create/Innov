...
- [ ] T-37.9 — **Insumo sem preço na UF ainda entra valendo zero.** Em SP, 1.996 dos 4.876 insumos não têm preço publicado — a planilha deixa a célula vazia porque não houve coleta mínima —, e 4.677 itens analíticos (10,6%) apontam para eles. O catálogo já os exclui, mas dentro da composição o leitor grava `unitCost: 0`, e a migration fecha a porta: `greatest(coalesce(nullif(v_item ->> 'unitCost','')::numeric, 0), 0)` converte qualquer `null` honesto em zero. Representar ausência como ausência exige coluna própria em `cost_composition_items` (a aba Analítico já traz a `Situação`: COM PREÇO, SEM PREÇO, COM CUSTO, SEM CUSTO, EM ESTUDO) e tela que mostre a composição como incompleta em vez de barata
