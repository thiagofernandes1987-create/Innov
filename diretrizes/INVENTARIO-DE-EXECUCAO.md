...
- [ ] T-73.3 — **V051**: seleção que sobrevive à volta da server action, porque é o DOM que o formulário envia. **A premissa da tarefa estava errada, e a correção veio antes da entrega.** A vacina declarava prevenção automática por `verif26.mjs`, arquivo de um arnês que **nunca foi versionado aqui** — a varredura da S-73 contou esta vacina como protegida por causa dessa frase. Corrigida para dizer a verdade, e o `pnpm validate:prevencao-declarada` entrou para impedir a próxima. O alvo real está medido: há **um** `<select name=…>` controlado por estado dentro de componente com `useActionState` — o desta vacina, já corrigido; os outros 16 usam `defaultValue`, e campo não controlado não diverge do DOM. O portão não é para o que existe, é para o próximo `value={estado}`
- [ ] T-73.4 — **V054**: `Escape` fechando o menor contexto aberto, nunca o formulário. Mesmo caso da T-73.3: a prevenção declarada era `verif28.mjs`, que também não existe. **As duas vacinas de formulário que a S-73 contou como protegidas eram as duas que citavam um arnês ausente** — e são as do defeito mais caro, o que apaga o que a pessoa acabou de digitar. Alvo medido: nove componentes tratam `Escape`, sete por ouvinte de `document` (camada externa, o lado que sofre); só um é camada interna dispensável, e é o que já barra a propagação
