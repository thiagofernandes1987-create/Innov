...
begin
  ...
  if not exists(select 1 from public.rh_tsv_esocial_profiles where organization_id=v_org and employment_id=v_emp and category_code='721') then raise exception 'Perfil TSVE 721 não persistiu'; end if;
  begin
    insert into public.rh_tsv_esocial_profiles(organization_id,employment_id,category_code,sex,race_color,education_level,birth_country_code,nationality_country_code,street,street_number,postal_code,city_ibge_code,state_code,salary_unit,fgts_option_date,created_by)
      values(v_org,gen_random_uuid(),'701','M',3,'09','105','105','Rua','1','12000000','3554102','SP',5,'2026-08-01',v_user);
  exception when check_violation then v_failed:=true; when foreign_key_violation then v_failed:=true; end;
  if not v_failed then raise exception 'Categoria fora da vertical 721 deveria ser bloqueada'; end if;
  v_failed:=false;
  begin
    insert into public.rh_tsv_esocial_profiles(organization_id,employment_id,category_code,sex,race_color,education_level,birth_country_code,nationality_country_code,street,street_number,postal_code,city_ibge_code,state_code,salary_unit,fgts_option_date,created_by)
  ...
  raise notice 'RH TSVE — perfil 721, constraints e tenant FK aprovados.';
end$$;
