do $$
declare
 v_org uuid;v_user uuid;v_employment uuid;v_salary uuid;v_salary_v uuid;v_bonus uuid;v_bonus_v uuid;v_tax uuid;v_tax_v uuid;
...
begin
 ...
 insert into public.rh_rubrics(organization_id,code,name,payroll_kind,created_by) values(v_org,'BONUS_FORA_BASE','Bônus fora da base','EARNING',v_user) returning id into v_bonus;
 insert into public.rh_rubric_versions(organization_id,rubric_id,version,valid_from,formula_type,calculation_priority,created_by) values(v_org,v_bonus,1,'2026-01-01','MANUAL',20,v_user) returning id into v_bonus_v;
 ...
 insert into public.rh_payroll_inputs(organization_id,period_id,employment_id,rubric_version_id,amount,source_type,created_by) values(v_org,v_period,v_employment,v_bonus_v,1000,'MANUAL',v_user);
 ...
 if v_base_amount<>5000 then raise exception 'FOLHA V2 1 falhou: base seletiva esperada 5000, obtida %',v_base_amount;end if;
 if v_gross<>6000 or v_ded<>900 or v_net<>5100 then raise exception 'FOLHA V2 2 falhou: bruto %, desconto %, líquido %',v_gross,v_ded,v_net;end if;
 ...
 if jsonb_array_length((select composition from public.rh_payroll_bases where run_id=v_run and base_code='CP'))<>1 then raise exception 'FOLHA V2 4 falhou: composição incluiu linha fora da base';end if;

 raise notice 'RH folha V2 — base seletiva e progressivo aprovados.';
end$$;
