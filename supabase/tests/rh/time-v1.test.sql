do $$
declare
  v_org uuid;v_user uuid;v_emp uuid;v_est uuid;v_schedule uuid;v_policy uuid;v_employment uuid;v_time uuid;
...
begin
  ...
  -- 03/08: 9 horas trabalhadas => 1 hora extra.
  insert into public.rh_time_punches(organization_id,employment_id,punched_at,direction,created_by) values
  (v_org,v_employment,'2026-08-03 08:00:00+00','IN',v_user),(v_org,v_employment,'2026-08-03 12:00:00+00','OUT',v_user),
  (v_org,v_employment,'2026-08-03 13:00:00+00','IN',v_user),(v_org,v_employment,'2026-08-03 18:00:00+00','OUT',v_user);
  ...
  -- TESTE 2: marcação original não pode ser alterada.
  v_raised:=false;begin update public.rh_time_punches set direction='OUT' where organization_id=v_org limit 1;exception when others then if sqlerrm like '%imutável%' then v_raised:=true;end if;end;
  ...
  -- TESTE 4: sequência inválida gera pendência.
  ...
  insert into public.rh_time_punches(organization_id,employment_id,punched_at,direction,created_by) values(v_org,v_employment,'2026-08-05 08:00:00+00','OUT',v_user),(v_org,v_employment,'2026-08-05 17:00:00+00','IN',v_user);
  perform public.calculate_rh_time_period(v_time);
