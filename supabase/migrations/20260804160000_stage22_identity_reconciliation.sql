...
-- whatsapp_contacts permanece o contato canônico; channel_* guarda aliases técnicos.
...
set identity_fingerprint=encode(extensions.digest(
...
alter table public.channel_contact_identities alter column identity_fingerprint set not null;
...
create table if not exists public.channel_identity_alias_links (
  ...
  check(pair_key~'^[0-9a-f-]{36}:[0-9a-f-]{36}$'),
...
create unique index if not exists channel_identity_alias_pair_uidx
  on public.channel_identity_alias_links(organization_id,pair_key);
...
set search_path=public,pg_temp
as $$
declare p public.channel_contact_identities%rowtype;
declare a public.channel_contact_identities%rowtype;
begin
  select * into p from public.channel_contact_identities where id=new.primary_identity_id;
  select * into a from public.channel_contact_identities where id=new.alias_identity_id;
  if p.id is null or a.id is null then raise exception 'IDENTITY_NOT_FOUND'; end if;
  if p.organization_id<>new.organization_id or a.organization_id<>new.organization_id
     or p.account_id<>new.account_id or a.account_id<>new.account_id
     or p.contact_id<>new.contact_id or a.contact_id<>new.contact_id then
    raise exception 'IDENTITY_ALIAS_SCOPE_MISMATCH';
  end if;
  new.pair_key:=least(new.primary_identity_id,new.alias_identity_id)::text||':'||
    greatest(new.primary_identity_id,new.alias_identity_id)::text;
  return new;
end;
$$;
...
create or replace function public.observe_channel_contact_identity(
  p_account_id uuid,p_contact_id uuid,p_namespace text,p_external_id text,
  p_normalized_id text,p_device_id text default null,p_observed_at timestamptz default now()
)
...
begin
  ...
  select * into existing from public.channel_contact_identities identity
  where identity.organization_id=account_row.organization_id
    ...
    and identity.namespace=p_namespace and identity.normalized_id=btrim(p_normalized_id)
    and coalesce(identity.device_id,'')=coalesce(p_device_id,'') and identity.active
  for update;
  if existing.id is not null then
    update public.channel_contact_identities identity set
      last_observed_at=greatest(identity.last_observed_at,p_observed_at),
      link_status=case when identity.contact_id<>p_contact_id then 'CONFLICT' else identity.link_status end,
      identity_version=identity.identity_version+1,updated_at=now()
    where identity.id=existing.id returning * into result;
  ...
  insert into public.channel_contact_identities(
    ...
    account_row.organization_id,account_row.id,contact_row.id,account_row.provider_type,
    account_row.provider_account_id,p_namespace,p_external_id,btrim(p_normalized_id),
    nullif(p_device_id,''),'OBSERVED','OBSERVED',p_observed_at,p_observed_at,fingerprint
...
create or replace function public.confirm_channel_identity_alias(
  p_primary_identity_id uuid,p_alias_identity_id uuid,p_evidence jsonb default '{}'::jsonb
)
...
declare key text;
begin
  select * into p from public.channel_contact_identities where id=p_primary_identity_id for update;
  select * into a from public.channel_contact_identities where id=p_alias_identity_id for update;
  if p.id is null or a.id is null then raise exception 'IDENTITY_NOT_FOUND'; end if;
  if p.organization_id<>a.organization_id or p.account_id<>a.account_id then raise exception 'CROSS_SCOPE_ALIAS'; end if;
  if p.contact_id<>a.contact_id then raise exception 'CONTACT_CONFIRMATION_REQUIRED'; end if;
  if p.namespace=a.namespace then raise exception 'ALIAS_NAMESPACE_MUST_DIFFER'; end if;
  key:=least(p.id,a.id)::text||':'||greatest(p.id,a.id)::text;
  select * into result from public.channel_identity_alias_links
    where organization_id=p.organization_id and pair_key=key for update;
  if result.id is null then
    insert into public.channel_identity_alias_links(
      organization_id,account_id,contact_id,primary_identity_id,alias_identity_id,pair_key,
      status,confidence,evidence,confirmed_by,confirmed_at
    ) values(
      p.organization_id,p.account_id,p.contact_id,p.id,a.id,key,'CONFIRMED','CONFIRMED',
      coalesce(p_evidence,'{}'::jsonb),auth.uid(),now()
    ) returning * into result;
  else
    update public.channel_identity_alias_links set status='CONFIRMED',confidence='CONFIRMED',
      evidence=coalesce(p_evidence,'{}'::jsonb),confirmed_by=auth.uid(),confirmed_at=now(),updated_at=now()
    where id=result.id returning * into result;
  end if;
  update public.channel_contact_identities set confidence='CONFIRMED',link_status='CONFIRMED',
    verified_at=now(),identity_version=identity_version+1,updated_at=now()
  where id in (p.id,a.id);
  update public.whatsapp_contacts set identity_version=identity_version+1,updated_at=now()
    where id=p.contact_id;
  perform public.bump_channel_identity_cache(p.organization_id);
  return result;
end;
$$;

create or replace function public.merge_channel_contacts(
  p_source_contact_id uuid,p_target_contact_id uuid,
  p_expected_source_version bigint,p_expected_target_version bigint,p_reason text
)
...
begin
  ...
  if source_row.organization_id<>target_row.organization_id or source_row.account_id<>target_row.account_id then
    raise exception 'CROSS_SCOPE_MERGE';
  end if;
  if source_row.identity_version<>p_expected_source_version or target_row.identity_version<>p_expected_target_version then
    raise exception 'CONTACT_VERSION_CONFLICT';
  end if;
  if not source_row.active or not target_row.active then raise exception 'CONTACT_INACTIVE'; end if;

  update public.whatsapp_conversations set contact_id=target_row.id,updated_at=now()
    where contact_id=source_row.id;
  get diagnostics moved_conversations=row_count;

  for source_identity in select * from public.channel_contact_identities
    where contact_id=source_row.id and active for update
  loop
    duplicate_identity:=null;
    select * into duplicate_identity from public.channel_contact_identities identity
    where identity.contact_id=target_row.id and identity.active
      ...
      and coalesce(identity.device_id,'')=coalesce(source_identity.device_id,'') limit 1;
    if duplicate_identity.id is not null then
      update public.channel_contact_identities set active=false,link_status='REJECTED',
        superseded_by=duplicate_identity.id,identity_version=identity_version+1,updated_at=now()
      where id=source_identity.id;
      superseded_identities:=superseded_identities+1;
    else
      update public.channel_contact_identities set contact_id=target_row.id,
        identity_version=identity_version+1,updated_at=now() where id=source_identity.id;
      moved_identities:=moved_identities+1;
    end if;
  end loop;

  update public.channel_identity_alias_links set contact_id=target_row.id,updated_at=now()
    where contact_id=source_row.id;
  update public.whatsapp_contacts set active=false,merged_into_contact_id=target_row.id,
    identity_version=identity_version+1,updated_at=now() where id=source_row.id;
  update public.whatsapp_contacts set identity_version=identity_version+1,updated_at=now()
  ...
  insert into public.channel_contact_merge_audit(
    ...
    source_row.organization_id,source_row.id,target_row.id,p_expected_source_version,
    p_expected_target_version,moved_conversations,moved_identities,superseded_identities,
    p_reason,auth.uid()
