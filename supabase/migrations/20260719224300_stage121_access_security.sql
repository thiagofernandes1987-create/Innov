...
create policy role_defaults_admin_read
on public.role_module_defaults for select to authenticated
using (
  exists(
    select 1 from public.organization_memberships om
    where om.user_id=auth.uid()
      and om.active
      and om.role in ('SUPER_ADMIN','DIRECAO','ADMINISTRADOR')
  )
);
...
revoke all on function public.seed_organization_access(uuid) from public,anon;
revoke all on function public.seed_organization_access_trigger() from public,anon,authenticated;
...
grant execute on function public.seed_organization_access(uuid) to service_role;
grant execute on function public.seed_organization_access_trigger() to service_role;
