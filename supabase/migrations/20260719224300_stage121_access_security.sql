...
create policy app_modules_authenticated_read
...
using (
  public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[])
  or exists(
    select 1 from public.organization_memberships om
    where om.access_profile_id=id and om.user_id=auth.uid() and om.active
  )
);

create policy access_profiles_admin_write
on public.access_profiles for all to authenticated
using (public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[]))
with check (public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[]));

create policy profile_permissions_admin_read
on public.profile_module_permissions for select to authenticated
using (public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[]));

create policy profile_permissions_admin_write
on public.profile_module_permissions for all to authenticated
using (public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[]))
with check (public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[]));

create policy user_overrides_admin_or_self_read
on public.user_module_permission_overrides for select to authenticated
using (
  user_id=auth.uid()
  or public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[])
);

create policy user_overrides_admin_write
on public.user_module_permission_overrides for all to authenticated
using (public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[]))
with check (public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[]));

create policy project_overrides_admin_manager_or_self_read
on public.project_module_permission_overrides for select to authenticated
using (
  user_id=auth.uid()
  or public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[])
...
create policy project_overrides_admin_manager_write
on public.project_module_permission_overrides for all to authenticated
using (
  public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[])
  or public.can_manage_project(project_id)
)
...
create policy permission_events_admin_read
on public.permission_change_events for select to authenticated
using (public.has_org_role(organization_id,array['SUPER_ADMIN','DIRECAO','ADMINISTRADOR']::public.org_role[]));
...
using (
  ...
  or exists(
    ...
    join public.organization_memberships actor
     ...
     and actor.active
     and actor.role in ('SUPER_ADMIN','DIRECAO','ADMINISTRADOR')
    where target.user_id=profiles.id and target.active
  )
);

revoke all on function public.default_role_module_permission(public.org_role,text) from public,anon;
...
revoke all on function public.remove_user_module_permission_override(uuid,text) from public,anon;
...
grant execute on function public.remove_user_module_permission_override(uuid,text) to authenticated,service_role;
