-- CHECK em PostgreSQL aceita resultado NULL. Tornamos obrigatoriedades de
-- infoEstagiario explicitamente booleanas para 901/906.
    ...
    or (
      ...
      and (
        ...
        or (
          ...
          and education_institution_name is not null
          and nullif(trim(education_institution_name),'') is not null
          and education_institution_street is not null
          and nullif(trim(education_institution_street),'') is not null
          and education_institution_number is not null
          and nullif(trim(education_institution_number),'') is not null
          and education_institution_neighborhood is not null
          and nullif(trim(education_institution_neighborhood),'') is not null
          and (education_institution_postal_code is null or education_institution_postal_code ~ '^[0-9]{8}$')
          and (education_institution_city_ibge_code is null or education_institution_city_ibge_code ~ '^[0-9]{7}$')
          and (education_institution_state_code is null or education_institution_state_code ~ '^[A-Z]{2}$')
        )
      )
