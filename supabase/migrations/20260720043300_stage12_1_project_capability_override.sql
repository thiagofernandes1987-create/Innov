-- Etapa 12.1 — wrapper granular para exceções por obra.

create or replace function public.set_project_module_capability_override(
  p_project_id uuid,
  p_user_id uuid,
  p_module_key text,
...
declare
  ...
  v_current public.project_module_permission_overrides;
...
begin
  ...
  return public.set_project_module_permission_override(
    p_project_id,
    p_user_id,
    p_module_key,
    v_denied,
...
revoke all on function public.set_project_module_capability_override(uuid,uuid,text,text,text,timestamptz,text) from public,anon;
grant execute on function public.set_project_module_capability_override(uuid,uuid,text,text,text,timestamptz,text) to authenticated,service_role;
