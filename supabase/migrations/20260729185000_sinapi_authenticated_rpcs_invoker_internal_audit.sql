-- Correção pós-advisor: a ponte estreita de auditoria não deve aparecer em
-- `/rest/v1/rpc`. Ela vive em schema não exposto e só é chamada pela RPC
-- SECURITY INVOKER depois de todas as políticas e guards do domínio.
...
create or replace function internal.write_sinapi_budget_item_audit(
...
revoke all on function internal.write_sinapi_budget_item_audit(uuid) from public;
revoke all on function internal.write_sinapi_budget_item_audit(uuid) from anon;
grant execute on function internal.write_sinapi_budget_item_audit(uuid) to authenticated;
...
begin
  ...
  perform internal.write_sinapi_budget_item_audit(v_budget_item.id);
...
-- Remove a porta transitória do schema exposto e restaura a revogação do helper
-- genérico, que pode ter sido recriado por migration posterior.
revoke all on function public.write_sinapi_budget_item_audit(uuid) from public, anon, authenticated, service_role;
drop function public.write_sinapi_budget_item_audit(uuid);
