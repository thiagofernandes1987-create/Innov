...
as $$
  select
    case
      when p_role in ('SUPER_ADMIN','DIRECAO','ADMINISTRADOR')
        then (enum_range(null::public.app_access_level))[4]
...
as $$
  with authorized as (
    ...
    where organization_id=p_organization_id and user_id=p_user_id and (expires_at is null or expires_at>now())
  ), project_override as (
    ...
    where organization_id=p_organization_id and user_id=p_user_id and project_id=p_project_id
      and p_project_id is not null and (expires_at is null or expires_at>now())
  )
