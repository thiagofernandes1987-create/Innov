...
begin
  ...
  return jsonb_build_object(
    ...
    'opportunitiesByStage',(
      select coalesce(jsonb_object_agg(stage,jsonb_build_object('count',total,'value',value)),'{}'::jsonb)
      from (
    ...
    'followUpsDue',(
      select count(*) from public.crm_activities
      where organization_id=p_organization_id and completed_at is null and due_at<now()
    ),
    'weightedPipeline',(
    ...
    'wonValue',(
      ...
      from public.opportunities
      where organization_id=p_organization_id and stage='WON' and archived_at is null
    )
  );
end $$;
  ...
  return jsonb_build_object(
    ...
    'contacts',(
      select coalesce(jsonb_agg(jsonb_build_object(
        'id',contact.id,'fullName',contact.full_name,'roleTitle',contact.role_title,
        'email',contact.email,'phone',contact.phone,
        'preferredChannel',contact.preferred_channel,'primary',contact.is_primary
      ) order by contact.is_primary desc,contact.full_name),'[]'::jsonb)
    ...
    'projects',(
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'actualStart',project.actual_start,'actualEnd',project.actual_end
    ...
    'opportunities',(
      ...
      ) order by opportunity.created_at desc),'[]'::jsonb)
    ...
    'contracts',(
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'totalValue',case when v_sensitive then contract.consolidated_value else null end
      ) order by contract.created_at desc),'[]'::jsonb)
    ...
    'recentActivities',(
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'subject',activity.subject,'description',activity.description,
        'occurredAt',activity.occurred_at,'dueAt',activity.due_at,
...
begin
  ...
  return jsonb_build_object(
    'client',jsonb_build_object(
      'id',v_client.id,'legalName',v_client.legal_name,
      'tradeName',v_client.trade_name,'email',v_client.email,
    ...
    'projects',(
      ...
      where project.client_id=v_client.id and project.client_released_at is not null
    ),
    'tickets',(
      select coalesce(jsonb_agg(jsonb_build_object(
        'id',ticket.id,'code',ticket.code,'title',ticket.title,
...
begin
  ...
  return jsonb_build_object(
    ...
    'open',(
      ...
      where organization_id=p_organization_id and status not in ('CLOSED','CANCELLED')
    ),
    'overdue',(
      select count(*) from public.sac_tickets
      where organization_id=p_organization_id
...
begin
  ...
  return jsonb_build_object(
    ...
    'attachments',(
      ...
      where attachment.ticket_id=v_ticket.id and (v_internal or attachment.client_visible)
    ),
