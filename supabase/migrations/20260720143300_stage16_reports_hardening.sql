-- Etapa 16 — hardening de snapshots e correção das agregações de compras.
...
with task_stats as (
  select organization_id, project_id,
    ...
    coalesce(sum(progress * nullif(weight,0)) / nullif(sum(nullif(weight,0)),0),avg(progress),0)::numeric(8,2) as weighted_task_progress
  from public.project_tasks group by organization_id, project_id
), contract_stats as (
  select organization_id, project_id,
  ...
  from public.contracts where project_id is not null group by organization_id, project_id
), finance_stats as (
  ...
  from public.finance_entries entry join public.finance_installments installment on installment.entry_id=entry.id
  where entry.project_id is not null group by entry.organization_id, entry.project_id
), finance_approval_stats as (
  select entry.organization_id, entry.project_id,
    count(*) filter (where approval.status::text='PENDING')::integer as pending_finance_approvals
  from public.finance_approvals approval join public.finance_entries entry on entry.id=approval.entry_id
  where entry.project_id is not null group by entry.organization_id, entry.project_id
), procurement_order_stats as (
  select organization_id,project_id,
    coalesce(sum(total) filter (where status::text<>'CANCELED'),0)::numeric(18,2) as purchase_committed,
    count(*) filter (where status::text in ('ISSUED','PARTIALLY_RECEIVED'))::integer as open_purchase_orders,
    count(*) filter (where expected_at < current_date and status::text in ('ISSUED','PARTIALLY_RECEIVED'))::integer as overdue_purchase_orders
  from public.procurement_orders group by organization_id,project_id
), procurement_request_stats as (
  select organization_id,project_id,
    count(*) filter (where status::text not in ('RECEIVED','CANCELED'))::integer as open_purchase_requests
  from public.procurement_requests group by organization_id,project_id
), procurement_receipt_stats as (
  select organization_id,project_id,
    count(*) filter (where status::text in ('ACCEPTED','ACCEPTED_WITH_RESTRICTION'))::integer as accepted_receipts
  from public.procurement_receipts group by organization_id,project_id
), quality_stats as (
  ...
  from public.quality_form_responses response where response.project_id is not null
  ...
  from public.quality_form_assignments where project_id is not null group by organization_id,project_id
), survey_values as (
  select response.organization_id,response.project_id,
    case when (answer.value_json #>> '{}') ~ '^[0-9]+([.][0-9]+)?$' then (answer.value_json #>> '{}')::numeric else null end as nps_value
  ...
  select organization_id,project_id,
    case when count(nps_value)>0 then round(
      (100.0*count(*) filter (where nps_value>=9)/count(nps_value))-
      (100.0*count(*) filter (where nps_value<=6)/count(nps_value)),2
    ) else null end as nps
  from survey_values group by organization_id,project_id
), daily_stats as (
  select organization_id,project_id,
    count(*) filter (where log_date>=current_date-29)::integer as daily_logs_30d
  from public.daily_logs group by organization_id,project_id
), document_stats as (
  select organization_id,project_id,
    count(*) filter (where status::text<>'ARCHIVED')::integer as project_documents
  from public.project_documents group by organization_id,project_id
)
select project.id as project_id,project.organization_id,project.client_id,project.code,project.name,
  ...
  coalesce(task.total_tasks,0) as total_tasks,coalesce(task.completed_tasks,0) as completed_tasks,
  coalesce(task.blocked_tasks,0) as blocked_tasks,coalesce(task.overdue_tasks,0) as overdue_tasks,
  ...
  coalesce(procurement_order.purchase_committed,0)::numeric(18,2) as purchase_committed,
  coalesce(procurement_order.open_purchase_orders,0) as open_purchase_orders,
  coalesce(procurement_order.overdue_purchase_orders,0) as overdue_purchase_orders,
  coalesce(procurement_request.open_purchase_requests,0) as open_purchase_requests,
  coalesce(procurement_receipt.accepted_receipts,0) as accepted_receipts,
  coalesce(quality.quality_responses,0) as quality_responses,coalesce(quality.quality_approved,0) as quality_approved,
  coalesce(quality.quality_rejected,0) as quality_rejected,coalesce(quality.pending_quality_reviews,0) as pending_quality_reviews,
  ...
  survey.nps::numeric(8,2) as nps,coalesce(daily.daily_logs_30d,0) as daily_logs_30d,
  coalesce(document.project_documents,0) as project_documents
from public.projects project
left join lateral (
  select snapshot.planned_progress,snapshot.actual_progress,snapshot.snapshot_date
  from public.project_progress_snapshots snapshot where snapshot.project_id=project.id
...
left join procurement_order_stats procurement_order on procurement_order.project_id=project.id
left join procurement_request_stats procurement_request on procurement_request.project_id=project.id
left join procurement_receipt_stats procurement_receipt on procurement_receipt.project_id=project.id
...
left join daily_stats daily on daily.project_id=project.id
left join document_stats document on document.project_id=project.id
where project.archived_at is null;

create or replace view public.report_executive_kpis_v with (security_invoker=true) as
select organization_id,count(*)::integer as active_projects,
...
from public.report_project_kpis_v group by organization_id;
...
using (
  ...
  and (
    coalesce((metrics->>'financialVisible')::boolean,false)=false
    or public.has_module_permission(organization_id,'relatorios','READ',project_id,'sensitive')
  )
);
