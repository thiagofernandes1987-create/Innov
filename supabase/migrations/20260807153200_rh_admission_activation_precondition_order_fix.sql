...
begin
 ...
 -- Mantém a primeira barreira histórica: nenhuma ativação com checklist pendente.
 if exists(select 1 from public.rh_admission_checklist_items where admission_case_id=c.id and required and status not in('COMPLETED','WAIVED')) then
   raise exception 'Checklist obrigatório incompleto';
 end if;

 -- Nova barreira funcional: o vínculo só nasce quando o conjunto eSocial mínimo
 -- foi capturado. Isto não afirma que o Ambiente Nacional já aceitou o evento.
