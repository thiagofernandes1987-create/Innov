...
describe("eSocial periódicos e desligamento",()=>{
 ...
 it("gera e valida S-2299",()=>{const xml=official(buildS2299({...employer,cpf:"12345678901",registrationNumber:"MAT001",reasonCode:"01",terminationDate:"2026-08-20",noticeIndemnified:true,projectedNoticeEnd:"2026-09-19",pensionPercentage:null,pensionAmount:null,lines:[line],demonstrativeId:"RES001"}),"S-2299");expect(xml).toContain("evtDeslig");expect(xml).toContain("<detVerbas>");});
 ...
 it("gera e valida S-2399",()=>{const xml=official(buildS2399({...employer,cpf:"12345678901",categoryCode:"701",registrationNumber:"TSV001",terminationDate:"2026-08-20",reasonCode:"01",lines:[line],demonstrativeId:"TSV001"}),"S-2399");expect(xml).toContain("evtTSVTermino");expect(xml).toContain("<codCateg>701</codCateg>");});
});
