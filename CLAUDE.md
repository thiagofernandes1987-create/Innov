...
- `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm test:python` e **todos** os `pnpm validate:*` devem passar antes de qualquer afirmação de conclusão. São 50 validadores; rodar um subconjunto e declarar a bateria verde já custou caro — foi um dos 46 não executados que pegou o defeito.
